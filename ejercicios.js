...
function factorial(n) {
  ...
  let resultado = 1; // se suele usar "let" para resultados que cambian, en este caso el resultado se actualiza en cada paso, cada que se repite
...
function promedioArray(numeros) {
  ...
  if (numeros.length === 0) { //numeros.lenght = cantidad de elementos
    return 0;
  }
...
function invertirArray(arr) {
  ...
  for (let i = arr.length - 1; i >= 0; i--){
    invertido.push(arr[i]); ///esto empuja directamente a la tabla luego de que lo haya invertido
  }
  return invertido;
}
